...
def send_friend_request():
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute('SELECT 1 FROM "friends" WHERE "user_id1" = %s AND "user_id2" = %s', (sender_id, receiver_id))
            if cur.fetchone():
...
@auth_bp.route('/friend-requests/pending', methods=['GET', 'OPTIONS'])
@jwt_required()
@cross_origin()
...
@auth_bp.route('/friends', methods=['GET', 'OPTIONS'])
@jwt_required()
@cross_origin()
def get_friends():
    if request.method == 'GET':
        ...
        sql_query = """
            SELECT u.id, u.nickname, u.email, u.cumulative_score
            FROM friends f
        ...
        # Note: user_id is passed twice
        cur.execute(sql_query, (user_id, user_id))
        
        # The dictionary keys should match the columns you are selecting
        friends = [{"id": row[0], "nickname": row[1], "email": row[2], "cumulative_score": row[3]} for row in cur.fetchall()]
        
        cur.close()
        conn.close() # It's also a good practice to close the connection
